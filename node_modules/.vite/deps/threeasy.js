...
// node_modules/threeasy/build/threeasy.js
var ThreeasyAnimator = class {
  /**
...
var ThreeasyLoader = class {
  constructor(app, settings) {
    ...
    this.manager = new this.THREE.LoadingManager(
      () => this.settings.load(),
      (itemURL, itemsLoaded, itemsTotal) => this.progress(itemURL, itemsLoaded, itemsTotal)
    );
    this.setUpLoaders();
  }
  /**
...
var ThreeasyInteractions = class {
  constructor(app) {
    __publicField(this, "noInteractions", () => {
  ...
  interactions() {
    ...
    this.app.renderer.domElement.addEventListener("mousemove", (event2) => {
      if (this.noInteractions()) return;
      this.calculatePosition(event2);
      this.calculateIntersects("mouseMove", event2);
    });
    this.app.renderer.domElement.addEventListener("click", (event2) => {
      if (this.noInteractions()) return;
      this.calculatePosition(event2);
      this.calculateIntersects("click", event2);
    });
    this.app.renderer.domElement.addEventListener("mouseup", (event2) => {
      if (this.noInteractions()) return;
      this.calculatePosition(event2);
      this.calculateIntersects("mouseup", event2);
    });
  }
  calculatePosition() {
  ...
  calculateIntersects(interactionType, event2) {
    let intersectUUIDs = [];
    for (let i = 0; i < this.intersects.length; i++) {
      const el = this.intersects[i];
      intersectUUIDs.push(el.object.uuid);
      if (interactionType == "click") {
        const item = this.clicks.find((item2) => el.object.uuid == item2.el.uuid);
        if (item) {
          item.fn(event2, el);
        }
      }
    ...
    if (interactionType == "mouseMove") {
      this.hovers.forEach((hoverItem) => {
        const isCurrentlyHovered = intersectUUIDs.indexOf(hoverItem.el.uuid) > -1;
        ...
        if (isCurrentlyHovered && !wasPreviouslyHovered) {
          ...
          if (hoverItem.fns.enter) {
            hoverItem.fns.enter(event2, hoverItem.el);
          }
        }
        if (!isCurrentlyHovered && wasPreviouslyHovered) {
          hoverItem.el.hovered = false;
          if (hoverItem.fns.leave) {
            hoverItem.fns.leave(event2, hoverItem.el);
          }
        }
      });
    }
...
var Threeasy = class {
  ...
  constructor(THREE, settings) {
    ...
    if (this.settings.light) {
      this.light = new THREE.AmbientLight(16777215, 1);
...
//# sourceMappingURL=threeasy.js.map
